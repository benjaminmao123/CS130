...
GLuint loadShader(const char *vertexfilename, const char *fragmentfilename) {
  ...
  std::string s1 = getTextFile(vertexfilename);
  std::string s2 = getTextFile(fragmentfilename);
  
  const char * temps1 = s1.c_str();
  const char * temps2 = s2.c_str();
  ...
  glShaderSource(vertex_shader_id, 1, &temps1, NULL);
  glShaderSource(fragment_shader_id, 1, &temps2, NULL);
